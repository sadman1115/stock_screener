"""Page 5: AI 강화학습 — 백테스트 기반 패턴 분석 + 강화 모델 학습."""
...
from core.backtest_engine import load_backtest_result  # noqa: E402
from core.config import (  # noqa: E402
    CACHE_DIR,
    RESULT_MCAP_DEFAULT_BILLION,
    RESULT_MCAP_MIN_BILLION,
)
from core.ml_pattern import SURGE_FEATURE_NAMES_KR  # noqa: E402
from core.reinforcement import (  # noqa: E402
    _extract_version_metrics,
    analyze_backtest_patterns,
    apply_evolution_version,
    generate_reinforced_rules,
    list_evolution_versions,
    list_reinforced_rule_versions,
    load_evolution_report,
    load_evolution_version,
    load_reinforced_backtest,
    load_reinforced_result,
    load_reinforced_rules,
    reset_reinforced_cache,
    rollback_reinforced_rules,
    run_evolution_batch,
    run_reinforced_backtest,
    save_evolution_report,
    save_reinforced_backtest,
    save_reinforced_result,
    save_reinforced_rules,
...
st.set_page_config(page_title="AI 강화학습", page_icon="🔄", layout="wide")
apply_mobile_styles()
...
# Section 0: 일괄 실행 / 재실행
# ══════════════════════════════════════════════════════
_bt_for_batch = load_backtest_result()

_existing_rules = load_reinforced_rules()
...
if _bt_for_batch is not None:
    # ── 학습 목표 파라미터 표시 (AI급등주 백테스트 설정 기반) ──
    _disp_tp_bt = _base_cfg.get("label_target_pct", 0.20)
    _disp_hw_bt = _base_cfg.get("label_horizon_weeks", 4)
    ...
    if batch_btn:
        ...
        if _has_rules:
            ...
            try:
                _batch_analysis = analyze_backtest_patterns(_bt_for_batch)
                st.session_state["pattern_analysis"] = _batch_analysis
            ...
            if not _batch_failed:
                ...
                _batch_progress.progress(5)
                try:
                    _batch_rules = generate_reinforced_rules(
                        _batch_analysis, _bt_for_batch
                    )
                    save_reinforced_rules(_batch_rules)
                    st.session_state["reinforced_rules"] = _batch_rules
                except Exception as e:
        ...
        if not _batch_failed:
            ...
            try:
                _batch_rf_res = train_reinforced_model(
                    universe_df=universe_df,
                    date_str=date_str,
                    rules=_batch_rules,
                    progress_callback=_batch_cb_train,
                )
                save_reinforced_result(_batch_rf_res)
                st.session_state["reinforced_result"] = _batch_rf_res
            except Exception as e:
        ...
        if not _batch_failed:
            ...
            try:
                _batch_bt_result = run_reinforced_backtest(
                    universe_df=universe_df,
                    date_str=date_str,
                    rules=_batch_rules,
                ...
                save_reinforced_backtest(_batch_bt_result)
                st.session_state["reinforced_bt_result"] = _batch_bt_result
            except Exception as e:
...
bt_result = load_backtest_result()
...
if analyze_btn:
    with st.spinner("패턴 분석 중..."):
        try:
            analysis = analyze_backtest_patterns(bt_result)
...
if analysis:
    ...
    if gen_rules_btn:
        with st.spinner("규칙 생성 중..."):
            try:
                rules = generate_reinforced_rules(analysis, bt_result)
                save_path = save_reinforced_rules(rules)
                st.session_state["reinforced_rules"] = rules
...
# 저장된 규칙 로드
saved_rules = st.session_state.get("reinforced_rules") or load_reinforced_rules()
if saved_rules:
    st.session_state["reinforced_rules"] = saved_rules
...
if saved_rules:
    ...
    if reinforce_btn:
        ...
        try:
            rf_res = train_reinforced_model(
                universe_df=universe_df,
                date_str=date_str,
                rules=saved_rules,
                progress_callback=_rcb,
            )
            save_reinforced_result(rf_res)
            st.session_state["reinforced_result"] = rf_res
...
# 결과 표시
rf_result = st.session_state.get("reinforced_result") or load_reinforced_result()

if rf_result:
    st.divider()
    st.header("🏆 강화학습 Top 20")
    ...
    # ── 메타 정보 배너 ──
    _meta_parts = []
    _meta_mcap = _base_cfg.get("min_market_cap", 50_000_000_000)
    _meta_mcap_eok = int(_meta_mcap / 1e8) if _meta_mcap else 500
    _meta_parts.append(f"시총 {_result_mcap_b:,}억 이상")
    _meta_tp = _base_cfg.get("label_target_pct", 0.20)
    _meta_hw = _base_cfg.get("label_horizon_weeks", 4)
    _meta_sp = _base_cfg.get("label_stop_pct", 0.08)
    ...
    if top20:
        ...
        with _dl_c1:
            ...
            st.download_button(
                "📥 Excel",
                _xl_buf.getvalue(),
                f"AI강화학습_top{len(_dl_df)}_{date_str}.xlsx",
        ...
        with _dl_c2:
            ...
            st.download_button(
                "📥 CSV",
                "\n".join(_csv_lines),
                f"AI강화학습_top{len(_dl_df)}_{date_str}.csv",
                "text/csv",
                use_container_width=True,
            )
        ...
        if ai_surge_key and ai_surge_key in existing_ai:
            st.subheader("🔄 기존 AI급등주 vs 강화학습 비교")
...
# ══════════════════════════════════════════════════════
# Section 4: 강화학습 수익률검사
# ══════════════════════════════════════════════════════
st.divider()
st.header("📊 강화학습 수익률검사")
...
if saved_rules:
    ...
    with st.expander("설정", expanded=True):
        if _base_cfg:
            st.caption("기본값: 기존 AI급등주 백테스트 설정에서 자동 로드됨")
    ...
    if rf_bt_btn:
        ...
        try:
            ...
            rf_bt_result = run_reinforced_backtest(
                universe_df=universe_df,
                date_str=date_str,
                rules=saved_rules,
            ...
            save_reinforced_backtest(rf_bt_result)
            st.session_state["reinforced_bt_result"] = rf_bt_result
...
# ══════════════════════════════════════════════════════
# Section 5: 기존 vs 강화학습 성과 비교
# ══════════════════════════════════════════════════════
rf_bt_result = (
    st.session_state.get("reinforced_bt_result") or load_reinforced_backtest()
)
base_bt_result = bt_result  # Section 1에서 이미 로드됨

if rf_bt_result and base_bt_result:
    st.divider()
    st.header("⚖️ 기존 AI급등주 vs 강화학습 성과 비교")
    ...
    if apply_btn:
        with st.spinner("규칙 업데이트 중... (패턴 재분석 → 새 규칙 생성)"):
            try:
                ...
                new_analysis = analyze_backtest_patterns(rf_bt_result)
                # 2. 새 규칙 생성 (version +1)
                new_rules = generate_reinforced_rules(new_analysis, rf_bt_result)
                new_rules["version"] = _cur_ver + 1
                # 3. 저장
                save_path = save_reinforced_rules(new_rules)
                # 4. 세션 업데이트
                st.session_state["reinforced_rules"] = new_rules
                st.session_state["pattern_analysis"] = new_analysis
                st.success(
                    f"규칙 v{_cur_ver + 1} 생성 완료! ({save_path.name}) "
                    f"→ '강화학습 수익률검사'를 다시 실행하여 개선 효과를 확인하세요."
                )
                st.rerun()
            except Exception as e:
    ...
    with st.expander("📈 진화 이력 / 버전 관리", expanded=False):
        _rule_versions = list_reinforced_rule_versions()
        ...
        if _rule_versions:
            ...
            if st.button(
                ...
                try:
                    restored = rollback_reinforced_rules(_sel_file)
                    st.session_state["reinforced_rules"] = restored
                    st.session_state.pop("reinforced_result", None)
                    st.session_state.pop("reinforced_bt_result", None)
                    st.success(
        ...
        with _rc2:
            if st.button(
                ...
                n_deleted = reset_reinforced_cache()
                st.session_state.pop("reinforced_rules", None)
                st.session_state.pop("reinforced_result", None)
                st.session_state.pop("reinforced_bt_result", None)
                st.session_state.pop("pattern_analysis", None)
                st.success(
...
# ──────────────────────────────────────────────
st.divider()
st.header("🧬 일괄 진화 (v1 ~ vN)")
st.caption(
    "기본 백테스트를 기반으로 v1→v2→...→v(N)까지 순차 진화하고, "
    "각 버전의 수익률검사를 모두 실행하여 최적 버전을 찾습니다."
)
...
_evo_c4, _evo_c5 = st.columns(2)
with _evo_c4:
    _evo_horizon = st.selectbox(
        "수익목표 기간(주)",
        [3, 4, 6, 8],
        index=1,
        key="evo_horizon",
...
# 기존 완료 버전 확인
_existing_versions = list_evolution_versions()
_existing_count = len(_existing_versions)
if _existing_count > 0:
    st.info(
        f"📂 기존 진화 버전: **v1 ~ v{_existing_count}** ({_existing_count}개 완료)\n\n"
        + (
            f"→ N={_evo_n} 선택 시: "
            + (
                f"**v{_existing_count + 1}부터 v{_evo_n}까지 이어서 실행** "
                f"({_evo_n - _existing_count}개 추가)"
                if _evo_n > _existing_count
                else f"**이미 v{_existing_count}까지 완료** — 기존 리포트를 표시합니다."
            )
        )
    )

# 시작 버튼
_need_run = _evo_n > _existing_count
_evo_btn_label = (
    f"🧬 v{_existing_count + 1}~v{_evo_n} 이어서 진화 ({_evo_n - _existing_count}개)"
    if _existing_count > 0 and _need_run
    else f"🧬 v1~v{_evo_n} 일괄 진화 시작"
)
_evo_btn = st.button(
    _evo_btn_label,
    type="primary",
    use_container_width=True,
    key="evo_start_btn",
    disabled=not _need_run and _existing_count > 0,
)

if _evo_btn:
    # 기본 백테스트 결과 확인
    _base_bt = st.session_state.get("backtest_result")
    if _base_bt is None:
        _base_bt = load_backtest_result(date_str)
    if _base_bt is None:
        st.error(
            "기본 백테스트 결과가 없습니다. "
            "먼저 **AI수익률검사** 페이지에서 백테스트를 실행하세요."
        ...
        if universe_df is None or universe_df.empty:
            st.error("종목 데이터를 로드할 수 없습니다.")
        else:
            _evo_progress = st.progress(0)
            _evo_status = st.empty()

            def _evo_cb(msg, pct=None):
                if pct is not None:
                    _evo_progress.progress(min(int(pct), 100))
                _evo_status.text(msg)

            with st.spinner(
                f"v1-v{_evo_n} 일괄 진화 실행 중... " f"(각 버전당 수분-수십분 소요)"
            ):
                try:
                    evo_report = run_evolution_batch(
                        universe_df=universe_df,
                        date_str=date_str,
                        n_versions=_evo_n,
                        base_backtest=_base_bt,
                        progress_callback=_evo_cb,
                        train_window_years=_evo_window,
                        top_k=_evo_topk,
                        num_boost_round=_evo_boost,
                        label_horizon_weeks=_evo_horizon,
                    )
                    st.session_state["evolution_report"] = evo_report
                    _evo_progress.progress(100)
                    _evo_status.empty()
                    st.success(
                        f"일괄 진화 완료! {evo_report['completed']}개 버전 생성, "
                        f"최적: v{evo_report['best_version']}"
                    )
                    st.rerun()
                except Exception as e:
                    st.error(f"일괄 진화 오류: {e}")
                    import traceback

                    st.code(traceback.format_exc())

# 리포트 표시 (파일 → 없으면 기존 버전으로 복구)
_evo_report = st.session_state.get("evolution_report")
if _evo_report is None:
    _evo_report = load_evolution_report()
if _evo_report is None and _existing_count > 0:
    # 리포트 파일 없지만 버전 파일은 있음 → 자동 복구
    _rebuild_versions = []
    for _rv in range(1, _existing_count + 1):
        _rev = load_evolution_version(_rv)
        if _rev and "backtest" in _rev:
            _rm = _extract_version_metrics(_rev["backtest"], _evo_horizon)
            _rebuild_versions.append(
                {
                    "version": _rv,
                    "rules_path": f"reinforced_rules_evo_v{_rv}.pkl",
                    "backtest_path": f"reinforced_backtest_evo_v{_rv}.pkl",
                    "metrics": _rm,
                }
            )
    if _rebuild_versions:
        _rk = sorted(
            _rebuild_versions,
            key=lambda x: x["metrics"]["avg_weekly_return"],
            reverse=True,
        )
        _evo_report = {
            "n_versions": _existing_count,
            "completed": len(_rebuild_versions),
            "date_str": date_str,
            "label_horizon_weeks": _evo_horizon,
            "versions": _rebuild_versions,
            "ranking": [v["version"] for v in _rk],
            "best_version": _rk[0]["version"],
            "created_at": "복구됨",
        }
        save_evolution_report(_evo_report)
        st.toast("기존 버전 파일로 리포트를 복구했습니다.")
if _evo_report:
    st.session_state["evolution_report"] = _evo_report

if _evo_report and _evo_report.get("versions"):
    st.subheader("📊 진화 리포트")
    st.caption(
        f"총 {_evo_report['completed']}개 버전 | "
        f"기준일: {_evo_report.get('date_str', '?')} | "
        f"보유기간: {_evo_report.get('label_horizon_weeks', 4)}주 | "
        f"생성: {_evo_report.get('created_at', '?')[:19]}"
    )

    # 랭킹 테이블
    _evo_rows = []
    for rank, v_data in enumerate(
        sorted(
            _evo_report["versions"],
            key=lambda x: x["metrics"]["avg_weekly_return"],
            reverse=True,
        ),
        1,
        ...
        _evo_rows.append(
            {
                "순위": rank,
                "버전": f"v{v_data['version']}",
                "평균주간수익률": f"{m['avg_weekly_return'] * 100:+.2f}%",
                "1Y수익률": f"{m.get('avg_weekly_return_1Y', 0) * 100:+.2f}%",
                "6M수익률": f"{m.get('avg_weekly_return_6M', 0) * 100:+.2f}%",
                "3M수익률": f"{m.get('avg_weekly_return_3M', 0) * 100:+.2f}%",
                "1M수익률": f"{m.get('avg_weekly_return_1M', 0) * 100:+.2f}%",
                "분할수익": f"{(m['realistic_return'] - 1) * 100:+.1f}%",
                "일괄수익": f"{(m.get('lumpsum_return', 1.0) - 1) * 100:+.1f}%",
                "1Y실제": f"{(m.get('realistic_return_1Y', 1.0) - 1) * 100:+.1f}%",
                "6M실제": f"{(m.get('realistic_return_6M', 1.0) - 1) * 100:+.1f}%",
                "3M실제": f"{(m.get('realistic_return_3M', 1.0) - 1) * 100:+.1f}%",
                "1M실제": f"{(m.get('realistic_return_1M', 1.0) - 1) * 100:+.1f}%",
                "승률": f"{m['win_rate'] * 100:.1f}%",
                "샤프": f"{m['sharpe']:.2f}",
    ...
    st.dataframe(
        pd.DataFrame(_evo_rows),
        use_container_width=True,
        hide_index=True,
        height=min(700, len(_evo_rows) * 35 + 38),
    )

    # 성과 비교 차트
    import plotly.graph_objects as go

    _chart_versions = sorted(_evo_report["versions"], key=lambda x: x["version"])
    _v_labels = [f"v{v['version']}" for v in _chart_versions]
    _v_returns = [v["metrics"]["avg_weekly_return"] * 100 for v in _chart_versions]
    _v_colors = [
        "#4CAF50" if v["version"] == _evo_report["best_version"] else "#1976D2"
        for v in _chart_versions
    ]
    ...
    fig_evo = go.Figure(
        go.Bar(
            x=_v_labels,
            y=_v_returns,
            marker_color=_v_colors,
            text=[f"{r:+.2f}%" for r in _v_returns],
            textposition="outside",
        )
    )
    fig_evo.update_layout(
        title="버전별 평균 주간수익률 비교",
        xaxis_title="버전",
        yaxis_title="평균 주간수익률 (%)",
        height=350,
    ...
    fig_evo2 = go.Figure()
    for label, key in [
        ("전체", "avg_weekly_return"),
        ("1Y", "avg_weekly_return_1Y"),
        ("6M", "avg_weekly_return_6M"),
        ("3M", "avg_weekly_return_3M"),
    ]:
        _vals = [v["metrics"][key] * 100 for v in _chart_versions]
        fig_evo2.add_trace(
            go.Scatter(
                x=_v_labels,
                y=_vals,
    ...
    _sel_options = [
        f"v{v['version']} — "
        f"수익률: {v['metrics']['avg_weekly_return'] * 100:+.2f}% | "
        f"실제: {(v['metrics']['realistic_return'] - 1) * 100:+.1f}% | "
        f"승률: {v['metrics']['win_rate'] * 100:.1f}%"
        for v in _ranked
    ]
    _sel_choice = st.selectbox(
        "적용할 버전 선택 (수익률순)",
        _sel_options,
        key="evo_version_select",
    )
    _sel_v = _ranked[_sel_options.index(_sel_choice)]["version"]

    if st.button(
        f"✅ v{_sel_v} 규칙을 최신(latest)으로 적용",
        type="primary",
        use_container_width=True,
        key="evo_apply_btn",
    ):
        try:
            applied_rules = apply_evolution_version(_sel_v)
            st.session_state["reinforced_rules"] = applied_rules
            st.session_state.pop("reinforced_result", None)
            st.session_state.pop("reinforced_bt_result", None)
            st.success(
                f"v{_sel_v} 적용 완료! "
                f"'강화 모델 학습' 버튼으로 해당 규칙 기반 예측을 실행하세요."
            )
            st.rerun()
        except Exception as e:
            st.error(f"버전 적용 실패: {e}")
