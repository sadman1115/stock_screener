...
from .ml_pattern import SURGE_FEATURE_NAMES, SURGE_FEATURE_NAMES_KR

REINFORCED_DIR = CACHE_DIR / "reinforced"
...
def analyze_backtest_patterns(backtest_result: dict) -> dict:
...
def generate_reinforced_rules(
    ...
    v1도 분석 기반 규칙을 가지므로 baseline과 다른 결과 산출.
    ...
    if version is not None:
        ...
        prev_rules = load_reinforced_rules()
...
def _suggest_lgb_params(summary: dict, version: int = 1) -> dict:
    ...
    # 승률 높지만 수익률 낮으면 → 세밀한 학습
    if win_rate > 0.52 and avg_ret < 0.01:
        lr = 0.03
        num_leaves = 25

    # 승률 낮으면 → 과적합 가능성, 정규화 강화
    if win_rate < 0.45:
        l1 = 0.3
        l2 = 1.5
        ff = 0.7
    ...
    return {
        ...
        "lambda_l2": l2,
        "feature_fraction": ff,
        "seed": 42 + version,  # 버전별 다른 랜덤 시드 → 탐색 다양성
    }
...
def _select_excluded_features(
    ...
    다른 피처 조합으로 학습하게 한다.
    - 기본 threshold: 0.01 (거의 무의미한 피처)
    - 버전이 올라갈수록 threshold 소폭 상승 (0.01 → 최대 0.03)
    - 최대 제외 비율도 버전에 따라 5% → 최대 10%
    """
    if not corr or len(available_feats) < 15:
        return []

    # 버전별 차등 threshold: 0.01, 0.012, 0.014, ... (최대 0.03)
    ...
    # 버전별 최대 제외 비율: 5% + version*0.5% (최대 10%)
...
def train_reinforced_model(
    universe_df: pd.DataFrame,
    date_str: str,
    rules: dict,
    progress_callback=None,
) -> dict:
    """강화된 규칙으로 모델 학습 + Top 20 예측.

    Args:
        universe_df: 전체 종목 DataFrame
        date_str: 기준일 (YYYYMMDD)
        rules: generate_reinforced_rules() 결과
    ...
    ohlcv_dict_all = {}  # 전종목 OHLCV (predict_current_surge용)
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            # 예측용 OHLCV 저장 (predict_current_surge는 ≥60일이면 처리 가능)
    ...
    # ── v1: 기존 AI급등주 캐시 모델을 그대로 사용 (baseline과 100% 동일) ──
    if is_v1:
        from .ml_pattern import load_model, predict_current_surge

        # target_pct=0.25 (25%급등주) 우선, 없으면 0.30, 없으면 기본 모델
        cached = None
        for tp in [0.25, 0.30, None]:
            cached = load_model(mode="surge", target_pct=tp)
            if cached is not None:
                _cb(
                    f"v1: 기존 AI급등주 캐시 모델 사용 "
        ...
        if cached is not None:
            _cb("전종목 예측 중 (AI급등주 모델)...", 60)
            pred_df = predict_current_surge(
                cached,
                ohlcv_dict_all,
            ...
            _cb("강화학습 완료 (v1: AI급등주 모델 동일)", 100)
            return {
                "top20": top20,
                "model_info": {
        ...
        _cb("⚠️ AI급등주 캐시 모델 없음 → 새로 학습합니다", 48)
    ...
    # ── 예측: predict_current_surge() 호출 (AI급등주와 동일한 코드 경로) ──
    _cb("전종목 예측 중...", 75)
    from .ml_pattern import predict_current_surge
...
def run_reinforced_backtest(
    ...
    _cb(
        f"강화학습 백테스트: {bt_start_str} ~ {bt_end_str} | "
    ...
    _cb(
        f"강화학습 백테스트 완료 ({len(daily_results)}주, {total_elapsed / 60:.1f}분)",
        90,
    )
...
def save_reinforced_rules(rules: dict) -> Path:
    """강화학습 규칙을 pickle로 저장."""
    REINFORCED_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath = REINFORCED_DIR / f"reinforced_rules_{ts}.pkl"
    with open(filepath, "wb") as f:
        pickle.dump(rules, f)
    # latest 파일도 동시 저장
    latest = REINFORCED_DIR / "reinforced_rules_latest.pkl"
    with open(latest, "wb") as f:
        pickle.dump(rules, f)
    return filepath


def load_reinforced_rules() -> dict | None:
    """저장된 규칙 로드 (latest)."""
    latest = REINFORCED_DIR / "reinforced_rules_latest.pkl"
    if latest.exists():
...
def save_reinforced_result(result: dict) -> Path:
    """강화학습 예측 결과 저장."""
    REINFORCED_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath = REINFORCED_DIR / f"reinforced_result_{ts}.pkl"
    with open(filepath, "wb") as f:
        pickle.dump(result, f)
    latest = REINFORCED_DIR / "reinforced_result_latest.pkl"
    with open(latest, "wb") as f:
        pickle.dump(result, f)
    return filepath


def load_reinforced_result() -> dict | None:
    """강화학습 예측 결과 로드."""
    latest = REINFORCED_DIR / "reinforced_result_latest.pkl"
    if latest.exists():
        with open(latest, "rb") as f:
...
def save_reinforced_backtest(result: dict) -> Path:
    """강화학습 백테스트 결과 저장."""
    REINFORCED_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath = REINFORCED_DIR / f"reinforced_backtest_{ts}.pkl"
    with open(filepath, "wb") as f:
        pickle.dump(result, f)
    latest = REINFORCED_DIR / "reinforced_backtest_latest.pkl"
    with open(latest, "wb") as f:
        pickle.dump(result, f)
    return filepath


def load_reinforced_backtest() -> dict | None:
    """최신 강화학습 백테스트 결과 로드."""
    latest = REINFORCED_DIR / "reinforced_backtest_latest.pkl"
    if latest.exists():
        with open(latest, "rb") as f:
...
def reset_reinforced_cache() -> int:
    """강화학습 캐시 전체 초기화 (v1부터 재시작).

    Returns:
        삭제된 파일 수
    """
    if not REINFORCED_DIR.exists():
        return 0
    deleted = 0
    for f in REINFORCED_DIR.glob("*.pkl"):
        f.unlink()
        deleted += 1
    return deleted


def rollback_reinforced_rules(filepath: str) -> dict:
    """특정 버전의 규칙 파일을 latest로 복원.

    Args:
        filepath: 복원할 규칙 pkl 파일명 (예: reinforced_rules_20260223_002415.pkl)

    Returns:
        복원된 rules dict
    """
    src = REINFORCED_DIR / filepath
    if not src.exists():
...
def list_reinforced_rule_versions() -> list[dict]:
    ...
    versions = []
    for f in sorted(REINFORCED_DIR.glob("reinforced_rules_2*.pkl"), reverse=True):
        try:
...
_EVOLUTION_DIR = REINFORCED_DIR  # 같은 디렉토리 사용


def save_evolution_version(version: int, rules: dict, backtest: dict) -> dict:
    """v{N} 규칙+백테스트를 개별 파일로 저장."""
    _EVOLUTION_DIR.mkdir(parents=True, exist_ok=True)
    rules_path = _EVOLUTION_DIR / f"reinforced_rules_evo_v{version}.pkl"
    bt_path = _EVOLUTION_DIR / f"reinforced_backtest_evo_v{version}.pkl"
    with open(rules_path, "wb") as f:
        pickle.dump(rules, f)
    with open(bt_path, "wb") as f:
        pickle.dump(backtest, f)
    return {"rules_path": str(rules_path.name), "backtest_path": str(bt_path.name)}


def load_evolution_version(version: int) -> dict | None:
    """특정 버전의 규칙+백테스트 로드."""
    rules_path = _EVOLUTION_DIR / f"reinforced_rules_evo_v{version}.pkl"
    bt_path = _EVOLUTION_DIR / f"reinforced_backtest_evo_v{version}.pkl"
    ...
    with open(rules_path, "rb") as f:
        result["rules"] = pickle.load(f)
    if bt_path.exists():
        with open(bt_path, "rb") as f:
            result["backtest"] = pickle.load(f)
    return result


def apply_evolution_version(version: int) -> dict:
    """선택한 진화 버전을 latest로 적용."""
    evo = load_evolution_version(version)
    if evo is None:
        raise FileNotFoundError(f"진화 v{version} 파일이 없습니다.")
    # latest로 복사
    rules = evo["rules"]
    latest_rules = _EVOLUTION_DIR / "reinforced_rules_latest.pkl"
    with open(latest_rules, "wb") as f:
        pickle.dump(rules, f)
    bt = evo.get("backtest")
    if bt:
        latest_bt = _EVOLUTION_DIR / "reinforced_backtest_latest.pkl"
        with open(latest_bt, "wb") as f:
            pickle.dump(bt, f)
    return rules


def save_evolution_report(report: dict) -> Path:
    """일괄 진화 리포트 저장."""
    _EVOLUTION_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = _EVOLUTION_DIR / f"evolution_report_{ts}.pkl"
    with open(path, "wb") as f:
        pickle.dump(report, f)
    latest = _EVOLUTION_DIR / "evolution_report_latest.pkl"
    with open(latest, "wb") as f:
        pickle.dump(report, f)
    return path
...
def list_evolution_versions() -> list[dict]:
    """저장된 진화 버전 목록 + 요약 메트릭."""
    if not _EVOLUTION_DIR.exists():
        return []
    versions = []
    for f in sorted(_EVOLUTION_DIR.glob("reinforced_rules_evo_v*.pkl")):
        try:
            ...
            versions.append(
                {
                    "version": v_num,
                    "filename": f.name,
                    "created_at": rules.get("created_at", "?"),
                }
            )
        except Exception:
            pass
...
def _extract_version_metrics(bt_result: dict, label_horizon_weeks: int = 4) -> dict:
    """백테스트 결과에서 리포트용 메트릭 추출."""
    summary = bt_result.get("summary", {})
    hp = label_horizon_weeks
    sbp = summary.get("summary_by_period", {}).get(hp, {})
    sbs = summary.get("summary_by_subperiod", {})

    def _get_sub(sub_label, key, default=0.0):
        return sbs.get(sub_label, {}).get(hp, {}).get(key, default)

    return {
        "avg_weekly_return": sbp.get("avg_return", 0.0),
        "avg_weekly_return_1Y": _get_sub("1Y", "avg_return"),
        "avg_weekly_return_6M": _get_sub("6M", "avg_return"),
        "avg_weekly_return_3M": _get_sub("3M", "avg_return"),
        "avg_weekly_return_1M": _get_sub("1M", "avg_return"),
        "realistic_return": sbp.get("realistic_cum_return", 1.0),
        "realistic_return_1Y": _get_sub("1Y", "realistic_cum_return", 1.0),
        "realistic_return_6M": _get_sub("6M", "realistic_cum_return", 1.0),
        "realistic_return_3M": _get_sub("3M", "realistic_cum_return", 1.0),
        "realistic_return_1M": _get_sub("1M", "realistic_cum_return", 1.0),
        "lumpsum_return": sbp.get("lumpsum_cum_return", 1.0),
        "cumulative_return": sbp.get("cumulative_return", 1.0),
        "win_rate": sbp.get("win_rate", 0.0),
        "sharpe": sbp.get("sharpe_ratio", 0.0),
        "max_drawdown": sbp.get("max_drawdown", 0.0),
        "n_weeks": sbp.get("n_weeks", 0),
    }
...
def run_evolution_batch(
    ...
    """v1~v(n) 순차 진화 + 수익률검사 일괄 실행.

    Args:
        universe_df: 시총 포함 universe DataFrame
        date_str: 기준일 (YYYYMMDD)
        n_versions: 진화 세대 수 (기본 20)
        base_backtest: 기본 백테스트 결과 (None이면 캐시에서 로드)
        progress_callback: fn(msg, pct) 진행 상태 콜백
        train_window_years: 학습 윈도우 (년)
        top_k: Top K 종목 수
        trailing_stop_pct: 트레일링 스탑 비율
        stop_loss_pct: 손절 비율
        min_profit_pct: 최소 수익률
        num_boost_round: LGB 부스팅 라운드
        label_horizon_weeks: 수익목표 기간 (주)

    Returns:
        dict with versions, ranking, best_version
    """
    ...
    # 기본 백테스트 결과 확보
    if base_backtest is None:
        from .backtest_engine import load_backtest_result

        base_backtest = load_backtest_result(date_str)
    if base_backtest is None:
        raise ValueError(
            "기본 백테스트 결과가 없습니다. " "AI수익률검사를 먼저 실행하세요."
        )

    # 이미 완료된 버전 체크 (중단 후 재개 지원)
    completed_versions = []
    for v in range(1, n_versions + 1):
        evo = load_evolution_version(v)
        if evo and "backtest" in evo:
            completed_versions.append(v)
        else:
            break  # 연속 완료 버전만 인정
    ...
    for v in completed_versions:
        evo = load_evolution_version(v)
        metrics = _extract_version_metrics(evo["backtest"], label_horizon_weeks)
        paths = {
            "rules_path": f"reinforced_rules_evo_v{v}.pkl",
            "backtest_path": f"reinforced_backtest_evo_v{v}.pkl",
        }
        versions_data.append({"version": v, **paths, "metrics": metrics})
        _cb(f"v{v} 이전 결과 로드 완료", int(v / n_versions * 100))

    start_v = len(completed_versions) + 1
    # 이전 백테스트: 마지막 완료 버전 또는 base
    if completed_versions:
        last_evo = load_evolution_version(completed_versions[-1])
        prev_bt = last_evo["backtest"]
    else:
        prev_bt = base_backtest

    def _build_report(vdata, n_total):
        """현재까지 완료된 데이터로 리포트 생성."""
        rk = sorted(
            vdata,
            key=lambda x: x["metrics"]["avg_weekly_return"],
            reverse=True,
        )
        rk_order = [vv["version"] for vv in rk]
        best = rk_order[0] if rk_order else 0
        return {
            "n_versions": n_total,
            "completed": len(vdata),
            "date_str": date_str,
    ...
    def _make_bt_cb(ver, base_pct):
        """클로저 변수 캡처를 위한 팩토리."""

        def _bt_cb(msg, pct=None):
            bt_pct = base_pct + 15
            if pct is not None:
                bt_pct = base_pct + 15 + int(pct * 0.7 * (100 / n_versions) / 100)
            _cb(f"v{ver} {msg}", min(bt_pct, 99))

        return _bt_cb

    for v in range(start_v, n_versions + 1):
        overall_pct = int((v - 1) / n_versions * 100)
        _cb(f"━━ v{v}/{n_versions} 진화 시작 ━━", overall_pct)

        try:
            # Phase 1: 패턴 분석
            _cb(f"v{v} 패턴 분석 중...", overall_pct + 5)
            analysis = analyze_backtest_patterns(prev_bt)

            # Phase 2: 규칙 생성
            _cb(f"v{v} 학습규칙 생성 중...", overall_pct + 10)
            rules = generate_reinforced_rules(analysis, prev_bt, version=v)

            # Phase 3: 강화 백테스트 실행
            _cb(f"v{v} 강화 백테스트 실행 중...", overall_pct + 15)
            bt_result = run_reinforced_backtest(
                universe_df=universe_df,
                date_str=date_str,
                rules=rules,
                progress_callback=_make_bt_cb(v, overall_pct),
            ...
            # Phase 4: 저장
            paths = save_evolution_version(v, rules, bt_result)
            metrics = _extract_version_metrics(bt_result, label_horizon_weeks)
            versions_data.append({"version": v, **paths, "metrics": metrics})

            _cb(
                f"v{v} 완료 — 평균수익률: "
                f"{metrics['avg_weekly_return'] * 100:+.2f}%",
                int(v / n_versions * 100),
            )

            # 매 버전 완료 시 중간 리포트 저장 (중단 복원용)
            _interim = _build_report(versions_data, n_versions)
            save_evolution_report(_interim)

            # 다음 세대의 입력으로 사용
            prev_bt = bt_result

        except Exception as e:
            _cb(f"v{v} 실패: {e}", overall_pct)
            # 실패해도 지금까지 완료된 버전으로 중간 리포트 저장
            if versions_data:
                _interim = _build_report(versions_data, n_versions)
                save_evolution_report(_interim)
            break
    ...
    # 최종 리포트
    report = _build_report(versions_data, n_versions)
    save_evolution_report(report)
    _cb(
        f"━━ 일괄 진화 완료: {len(versions_data)}개 버전, "
        f"최고 v{report['best_version']} ━━",
        100,
    )
    return report
